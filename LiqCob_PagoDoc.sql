...
SELECT  cte_doc, tip_doc, fol_doc, ffis_doc, ser_doc, cia_doc, pla_doc, ffac_doc, sfac_doc, sta_doc, tpa_doc, uso_doc,
			sal_doc, fven_doc,vuelta_doc, fult_doc, car_doc, abo_doc
	FROM	doctos
	WHERE	fol_doc = 71168 AND cia_doc = '15' AND pla_doc = '85' AND 
			(vuelta_doc is null OR tip_doc = '03') AND sal_doc > 0.0;
	
select	*
from	doctos
where	fol_doc in(2023) and cte_doc = '08869'

select	*
from	mov_cxc
where	doc_mcxc in(561274) and pla_mcxc ='09' and cte_mcxc = '061549' and tpm_mcxc = '50'

select	saldo_cte, abono_cte, cargo_cte, fecuab_cte, * 
from	cliente
where   num_cte = '035397'

update	cliente
set		saldo_cte = 0.00, cargo_cte = 91141.89, abono_cte =91141.89, fecuab_cte = '2022-09-08'
where   num_cte = '035397'


select	sum(abo_doc),sum(car_doc)
from	doctos
where	cte_doc in('035397') and sta_doc = 'A'

select	*
from	doctos
where	cte_doc in('245845') and sta_doc = 'A'
